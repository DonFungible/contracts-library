...
async function initializeTests() {
    const contractName = 'DutchAuctionDrop';
...
describe('DutchAuctionDrop tests', function () {
    ...
    const expected = {
        ...
        startPrice: ethers.utils.parseEther('1'), // 1 ether
        endPrice: ethers.utils.parseEther('0.25'),
        priceDecreasePerInterval: ethers.utils.parseEther('0.05'), // Price reduction per drop
        startTime: 1688195714,
        priceDecreaseTimeInterval: 3600, // Time in seconds between price drops
    ...
    describe('mintDutchAuction', function () {
        it('should update minters balance after minting successfully', async function () {
            await contract
                ...
                .setAuctionParams(
                    expected.startPrice,
                    expected.endPrice,
                    expected.priceDecreasePerInterval,
                    expected.startTime,
                    expected.priceDecreaseTimeInterval
                );
            const currentPrice = await contract.getAuctionPrice();
            expect(await contract.balanceOf(addr1.address)).to.equal(0);
            await contract.connect(owner).toggleIsPublicMintOpen();
            await contract.connect(addr1).mintPublic(1, { value: currentPrice });
            expect(await contract.balanceOf(addr1.address)).to.equal(1);
        });
        xit('should mint when minter pays correct amount and mint is open', async function () {
            await contract.connect(owner).toggleIsPublicMintOpen();
            expect(await contract.connect(addr1).mintPublic(1, { value: expected.publicMintPrice })).to.be.ok;
        });
        xit('should mint any amount up to the maximum per transaction limit', async function () {
            await contract.connect(owner).toggleIsPublicMintOpen();
            expect(await contract.balanceOf(addr1.address)).to.equal(0);
            expect(
        ...
        xit('should revert when minter pays incorrect amount', async function () {
            await contract.connect(owner).toggleIsPublicMintOpen();
            await expect(contract.connect(addr1).mintPublic(1, { value: '0' })).to.be.revertedWithCustomError(contract, 'IncorrectPayment');
            await expect(contract.connect(addr1).mintPublic(1, { value: expected.publicMintPrice.mul(10) })).to.be.revertedWithCustomError(
                contract,
                'IncorrectPayment'
            );
        });
        xit('should revert when mint is not open', async function () {
            await expect(contract.connect(addr1).mintPublic(1, { value: '0' })).to.be.revertedWithCustomError(contract, 'MintNotStarted');
        });
        it('should revert when minting more than maximum per transaction', async function () {
            await contract.connect(owner).toggleIsPublicMintOpen();
            await expect(contract.connect(addr1).mintPublic(50, { value: expected.publicMintPrice.mul(50) })).to.be.revertedWithCustomError(
                contract,
                'ExceedsTxnLimit'
            );
        });
        xit('should revert if minting over max supply', async function () {
            await contract.connect(owner).toggleIsPublicMintOpen();
            await contract.connect(owner).mintReserve(expected.maxSupply);
            await expect(contract.connect(addr1).mintPublic(1, { value: expected.publicMintPrice })).to.be.revertedWithCustomError(
                contract,
                'ExceedsMaxSupply'
            );
            await expect(contract.connect(owner).mintPublic(2, { value: expected.publicMintPrice.mul(2) })).to.be.revertedWithCustomError(
                contract,
                'ExceedsMaxSupply'
            );
        });
    ...
    describe('mintAllowlist', function () {
        ...
        it('should mint when minter pays correct amount and mint is open', async function () {
            ...
            await contract.connect(owner).toggleIsAllowlistMintOpen();
            expect(await contract.connect(addr1).mintAllowlist(1, merkleProof, { value: expected.allowlistMintPrice })).to.be.ok;
        });
        ...
        it('should revert when minter pays incorrect amount', async function () {
            ...
            await contract.connect(owner).toggleIsAllowlistMintOpen();
            await expect(contract.connect(addr1).mintAllowlist(1, merkleProof, { value: '0' })).to.be.revertedWithCustomError(
                contract,
                'IncorrectPayment'
            );
            await expect(
    ...
    describe('mintReserve', function () {
        ...
        it('should revert if minting over max supply', async function () {
            await contract.connect(owner).mintReserve(expected.maxSupply);
            await expect(contract.connect(owner).mintReserve(1)).to.be.revertedWithCustomError(contract, 'ExceedsMaxSupply');
        });
        it('should revert when not called by owner', async function () {
            await expect(contract.connect(addr1).mintReserve(1)).to.be.revertedWithCustomError(contract, 'Unauthorized');
        });
    });
    context('Setter functions', function () {
        describe('setMaxSupply', function () {
            ...
            it('should revert when not called by owner', async function () {
                await expect(contract.connect(addr1).setMaxSupply(100)).to.be.revertedWithCustomError(contract, 'Unauthorized');
            });
        });
        describe('setMaxPublicMints', function () {
            ...
            it('should revert when not called by owner', async function () {
                await expect(contract.connect(addr1).setMaxPublicMints(10)).to.be.revertedWithCustomError(contract, 'Unauthorized');
            });
        });
        describe('setMaxAllowlistMints', function () {
            ...
            it('should revert when not called by owner', async function () {
                await expect(contract.connect(addr1).setMaxAllowlistMints(10)).to.be.revertedWithCustomError(contract, 'Unauthorized');
            });
        });
        describe('setPublicMintPrice', function () {
            ...
            it('should revert when not called by owner', async function () {
                await expect(contract.connect(addr1).setPublicMintPrice(ethers.utils.parseEther('2'))).to.be.revertedWithCustomError(
                    contract,
                    'Unauthorized'
                );
            });
        ...
        describe('setAllowlistMintPrice', function () {
            ...
            it('should revert when not called by owner', async function () {
                await expect(contract.connect(addr1).setAllowlistMintPrice(ethers.utils.parseEther('2'))).to.be.revertedWithCustomError(
                    contract,
                    'Unauthorized'
                );
            });
        ...
        describe('setMerkleRoot', function () {
            ...
            it('should revert when not called by owner', async function () {
                ...
                await expect(contract.connect(addr1).setMerkleRoot(newMerkleRoot)).to.be.revertedWithCustomError(contract, 'Unauthorized');
            });
        });
        describe('setBaseURI', function () {
            ...
            it('should revert when not called by owner', async function () {
                ...
                await expect(contract.connect(addr1).setBaseURI(newBaseURI)).to.be.revertedWithCustomError(contract, 'Unauthorized');
            });
        });
        describe('setDefaultRoyalty', function () {
            ...
            it('should revert when not called by owner', async function () {
                await expect(contract.connect(addr1).setDefaultRoyalty(addr1.address, 1000)).to.be.revertedWithCustomError(
                    contract,
                    'Unauthorized'
                );
            });
        ...
        describe('toggleIsPublicMintOpen', function () {
            ...
            it('should revert when not called by owner', async function () {
                await expect(contract.connect(addr1).toggleIsPublicMintOpen()).to.be.revertedWithCustomError(contract, 'Unauthorized');
            });
        });
        describe('toggleIsAllowlistMintOpen', function () {
            ...
            it('should revert when not called by owner', async function () {
                await expect(contract.connect(addr1).toggleIsAllowlistMintOpen()).to.be.revertedWithCustomError(contract, 'Unauthorized');
            });
        });
    });
    ...
    describe('Withdraw funds', function () {
        ...
        it('should revert if contract has empty balance', async function () {
            await expect(contract.connect(owner).withdrawAll()).to.be.revertedWithCustomError(contract, 'NoFundsToWithdraw');
        });
        it('should revert when not called by owner', async function () {
    ...
    describe('transferFrom', function () {
        it('should revert when not called by owner', async function () {
            ...
            await expect(contract.connect(addr1).transferFrom(owner.address, addr1.address, 1)).to.be.revertedWithCustomError(
                contract,
